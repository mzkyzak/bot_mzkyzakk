...
import { fetchGroupsSafe } from "../../src/lib/mzkyzak-jpm-helper.js";
import {
  getAutoJpmConfig,
  setAutoJpmConfig,
...
} from "../../src/lib/mzkyzak-auto-jpm.js";
import { getMimeType, getExtension } from "../../src/lib/mzkyzak-utils.js";
...
try {
  cachedThumb = getAssetBuffer("mzkyzak2");
} catch {}
...
async function startAutoJpmSession(m, sock, db) {
  ...
  return sendInteractiveMessage(m, sock, {
    title: `🔄 ${config.bot?.name || "Mzkyzak-AI"} AutoJPM`,
    body,
    footer: `${config.bot?.name || "Mzkyzak-AI"} AutoJPM`,
    buttons,
  });
}
...
async function handleSetDelay(m, sock, db, input) {
  ...
  if (!input) {
    ...
    return sendInteractiveMessage(m, sock, {
      title: `⏱️ ${config.bot?.name || "Mzkyzak-AI"} Delay`,
      body,
      footer: `${config.bot?.name || "Mzkyzak-AI"} JPM System`,
      buttons,
    });
  }
