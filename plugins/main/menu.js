import { getCaseCount, getCasesByCategory } from "../../case/mzkyzak.js";
import {
...
import {
  formatUptime,
  getTimeGreeting,
} from "../../src/lib/mzkyzak-formatter.js";
import {
  getCommandsByCategory,
  getCategories,
} from "../../src/lib/mzkyzak-plugins.js";
...
const pluginConfig = {
  // Trigger hot-reload to clear cached imports
  name: "menu",
...
async function formatTime(date) {
  const timeHelper = await import("../../src/lib/mzkyzak-time.js");
...
async function buildMenuText(
  ...
  txt += `    ᯓ INFO BOT\n`;
  txt += `╭  • Name : ${botConfig.bot?.name || "Mzkyzak-AI"}\n`;
  txt += `┆  • Author : ${botConfig.bot?.developer || "Owner"}\n`;
  txt += `┆  • Versi : ${botConfig.bot?.version || "1.2.0"}\n`;
  txt += `┆  • Type script : MZKYZAK x ${botConfig.bot?.developer || "Owner"}\n`;
...
function getContextInfo(
  ...
  const saluranName =
    botConfig.saluran?.name || botConfig.bot?.name || "Mzkyzak-AI";
  ...
  const ctx = {
    ...
    isForwarded: true,
    externalAdReply: {
      title: botConfig.bot?.name || "Mzkyzak-AI",
...
async function handler(m, { sock, config: botConfig, db, uptime }) {
  ...
  try {
    imageBuffer = fs.readFileSync(botConfig.assets["mzkyzak"])
    thumbBuffer = fs.readFileSync(botConfig.assets["mzkyzak2"])
  } catch (e) {
...
╭┈┈ NEED PANEL & SEWABOT? 
│ ◈ https://https://wa.me/6288901796549
╰┈┈┈┈┈┈┈┈
${readmore}${s}`
                  },
                  footer: {
      ...
      case 3:
        const content = {
          buttonsMessage: {
            ...
            locationMessage: {
              jpegThumbnail: await sharp(fs.readFileSync(config.assets["mzkyzak"])).resize(300, 170).toBuffer(),
              name: config.bot.name,
...
> 🍬 *Register*: ${user.isRegistered ? "Sudah" : "Belum"}`,
      ...
      case 4: {
        const thumbnail = await sharp(fs.readFileSync(config.assets["mzkyzak"])).resize(300, 300).toBuffer()
        const qvideo = {
          key: {
            fromMe: false,
            participant: m.sender
        ...
        const media4 = await prepareWAMessageMedia({
          video: fs.readFileSync(config.assets["mzkyzak-mp4"]),
          gifPlayback: true
        }, { upload: sock.waUploadToServer });
              ...
              interactiveMessage: {
                header: {
                  ...
                  locationMessage: {
                    degreesLatitude: 0,
                    degreesLongitude: 0,
                    name: config.bot?.name || "Mzkyzak-AI",
...
`
        ...
        const { getAssetBuffer } = await import("../../src/lib/mzkyzak-asset-manager.js");
        const imageBuffer = await getAssetBuffer("mzkyzak2");
        ...
        const { generateWAMessageFromContent } = await import("mzkyzak");
        const menuMedia = await prepareWAMessageMedia({
          image: await getAssetBuffer("mzkyzak")
        }, { upload: sock.waUploadToServer });

        const videoLive = await prepareWAMessageMedia({
          video: await getAssetBuffer("mzkyzak-mp4")
        }, { upload: sock.waUploadToServer });
    ...
    if (audioEnabled) {
      const audioUrl = botConfig.assets["mzkyzak-mp3"];
      try {
        switch (menuVariant) {
          case 1:
            try {
          ...
          case 2: {
            ...
            try {
              ...
              await sock.sendMessage(m.chat, {
                audio: fs.readFileSync(config.assets["mzkyzak-mp3"]),
                mimetype: "audio/mpeg",
                ptt: false,
              }, { quoted: qpoll });
            }
