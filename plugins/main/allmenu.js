...
import { generateWAMessageFromContent, prepareWAMessageMedia, proto } from "mzkyzak";
...
import { getCasesByCategory, getCaseCount } from "../../case/mzkyzak.js";
const pluginConfig = {
  name: "allmenu",
  ...
  try {
    switch (allmenuVariant) {
      ...
      case 2:
        ...
        try {
          media = await prepareWAMessageMedia({
            image: imageBuffer || fs.readFileSync(config.assets["mzkyzak"])
      ...
      case 5: {
        ...
        const thumbnail = await _sharp(fs.readFileSync(config.assets["mzkyzak"])).resize(300, 300).toBuffer()
        const qOrder = {
        ...
        const media4 = await prepareWAMessageMedia({ video: fs.readFileSync(config.assets["mzkyzak-mp4"]), gifPlayback: true }, { upload: sock.waUploadToServer });
      ...
      case 6: {
        ...
        let thumbnail = null;
        try {
          thumbnail = await _sharp(imageBuffer || fs.readFileSync(config.assets["mzkyzak"])).resize(300, 300).toBuffer();
        } catch (e) { }
        ...
        switch (audioVariant) {
          ...
          case 3: {
            const qtext = {
              ...
              message: {
                conversation: "setelin music donk mzkyzak/topik"
              }
            };
            await sock.sendMessage(m.chat, {
