...
const NEOXR_APIKEY = config.APIkey?.neoxr || "Milik-Bot-MzkyzakMD";
...
async function handler(m, { sock }) {
  ...
  try {
    const res = await axios.get(
      `https://firefly.maiku.my.id/api/pinterestvideo?apikey=MzkyzakNextGen&q=${encodeURIComponent(query)}`,
      {
        timeout: 60000,
      },
    );
