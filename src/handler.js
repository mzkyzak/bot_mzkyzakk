...
import { generateWAMessageFromContent, prepareWAMessageMedia } from "mzkyzak";
import { serialize, getCachedThumb } from "./lib/mzkyzak-serialize.js";
import { saluranCtx } from "./lib/mzkyzak-context.js";
import {
  getPlugin,
  getPluginCount,
  getAllPlugins,
  pluginStore,
  getAllCommandNames,
} from "./lib/mzkyzak-plugins.js";
import {
  findSimilarCommands,
  formatSuggestionMessage,
} from "./lib/mzkyzak-similarity.js";
import { getDatabase } from "./lib/mzkyzak-database.js";
import {
  formatUptime,
  createWaitMessage,
  createErrorMessage,
} from "./lib/mzkyzak-formatter.js";
import { getUptime } from "./connection.js";
import { logger, logMessage, c } from "./lib/mzkyzak-logger.js";
import {
  isLid,
  isLidConverted,
  lidToJid,
  ...
  savePersistentCache,
  getLidCacheSize,
} from "./lib/mzkyzak-lid.js";
import { hasActiveSession, getSession } from "./lib/mzkyzak-game-data.js";
import {
  levenshtein,
  formatAfkDuration,
  checkPermission,
  checkMode,
  getBotMode,
  shouldSilenceInGlobalSelfMode,
} from "./lib/mzkyzak-middleware.js";
import {
  handleAntilink,
  handleAntiJudol,
  handleAntiPhising,
  ...
  handleAntiHidetag,
  handleAntiSwGc,
} from "./lib/mzkyzak-group-protection.js";
import {
  debounceMessage,
  getCachedUser,
  getCachedGroup,
  getCachedSetting,
} from "./lib/mzkyzak-performance.js";
import {
  isJadibotOwner,
  isJadibotPremium,
  loadJadibotDb,
} from "./lib/mzkyzak-jadibot-database.js";
import { getActiveJadibots } from "./lib/mzkyzak-jadibot-manager.js";
import { handleCommand as handleCaseCommand } from "../case/mzkyzak.js";
import { RateLimiterMemory } from "rate-limiter-flexible";
import { games as mzkyzakGames } from "./lib/mzkyzak-games.js";
import fs from "fs";
import path from "path";
import { exec } from "child_process";
import axios from "axios";
import * as timeHelper from "./lib/mzkyzak-time.js";
const safe = (fn) => {
  try {
    return fn();
  } catch {
...
try {
  levelHelper = await import("./lib/mzkyzak-level.js");
} catch { }
try {
...
try {
  handleAutoAI = (await import("./lib/mzkyzak-auto-ai.js")).handleAutoAI;
} catch { }
try {
  handleAutoDownload = (await import("./lib/mzkyzak-auto-download.js"))
    .handleAutoDownload;
} catch { }
try {
  checkStickerCommand = (await import("./lib/mzkyzak-sticker-command.js"))
    .checkStickerCommand;
} catch { }
try {
  handleStickerReply = (await import("./lib/mzkyzak-sticker-reply.js"))
    .handleStickerReply;
} catch { }
try {
...
async function getSmartTriggerThumb() {
  ...
  try {
    const url = config.assets["mzkyzak2"];
    if (url) {
...
async function handleSmartTriggers(m, sock, db) {
  ...
  try {
    ...
    const saluranName = config.saluran?.name || config.bot?.name || "Mzkyzak-AI";
    const botName = config.bot?.name || "Mzkyzak-AI";
...
async function messageHandler(msg, sock, options = {}) {
  ...
  try {
    ...
    if (shouldSilenceInGlobalSelfMode(m, db)) {
      return;
    }
          ...
          if (buffer && buffer.length > 1000) {
            ...
            if (transcript) {
              ...
              if (bestMatch) {
                ...
                const { parseCommand } =
                  await import("./lib/mzkyzak-serialize.js");
    ...
    if (m.body?.startsWith(">>") && m.isOwner) {
      ...
      try {
        ...
        const execCode = new AsyncFunction(
          ...
          const { generateWAMessage, getBuffer, generateWAMessageFromContent, proto, generateMessageID } = await import('mzkyzak')
          const { exec: childExec } = await import('child_process')
          const { VERSION, Button, ButtonV2, Carousel, AIRich, } = await import('./lib/mzkyzak-builder.js')
      ...
      if (checkStickerCommand) {
        try {
          ...
          if (stickerCmd) {
            ...
            const { parseCommand } = await import("./lib/mzkyzak-serialize.js");
            const parsed = parseCommand(m.body, prefix);
            m.isCommand = parsed.isCommand;
            m.command = parsed.command;
            m.args = parsed.args;
    ...
    if (!plugin) {
      ...
      if (similarityEnabled) {
        ...
        if (suggestions.length > 0) {
          ...
          try {
            const { prepareWAMessageMedia } = await import("mzkyzak");
            const media = await prepareWAMessageMedia(
              { image: fs.readFileSync(config.assets["mzkyzak"]) },
                      ...
                      nativeFlowMessage: {
                        messageParamsJson: JSON.stringify({
                          ...
                          tap_target_configuration: {
                            title: " X ",
                            description: "Close",
                            canonical_url: "https://mzkyzak.site",
                            domain: "mzkyzak.example",
                            button_index: 0,
                          },
                        }),
...
async function groupHandler(update, sock) {
  try {
    ...
    if (getBotMode(db) === "self") {
      const publicGroups = db.setting("publicGroups") || [];
      if (!publicGroups.includes(groupJid)) {
        return;
      }
    }
    ...
    for (let participant of participants) {
      ...
      const saluranId = config.saluran?.id || "120363400911374213@newsletter";
      const saluranName =
        config.saluran?.name || config.bot?.name || "Mzkyzak-AI";
      ...
      const rankActions = {
        promote: {
          ...
          imgKey: "_promoteImg",
          imgPath: config.assets["mzkyzak-promote"],
          emoji: "🎉",
          label: "PROMOTE",
          text: (p, a) =>
        ...
        demote: {
          ...
          imgKey: "_demoteImg",
          imgPath: config.assets["mzkyzak-demote"],
          emoji: "📉",
          label: "DEMOTE",
          text: (p, a) =>
            ...
            viewOnceMessage: {
              message: {
                ...
                interactiveMessage: {
                  ...
                  footer: { text: config.bot?.name || "Mzkyzak-AI" },
                  ...
                  contextInfo: {
                    ...
                    forwardedNewsletterMessageInfo: {
                      ...
                      newsletterName: config.saluran?.name || config.bot?.name || "Mzkyzak-AI",
                      serverMessageId: 127,
                    },
                  },
                  nativeFlowMessage: {
                    messageParamsJson: JSON.stringify({
                      ...
                      tap_target_configuration: { title: " X ", description: "bomboclard", canonical_url: "https://mzkyzak.site", domain: "shop.example.com", button_index: 0 },
                    }),
