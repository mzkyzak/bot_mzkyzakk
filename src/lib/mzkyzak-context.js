...
const keys = [
  ["mzkyzak-games", (buf) => { gameThumbBuffer = buf; }],
  ["mzkyzak-rpg", (buf) => { rpgThumbBuffer = buf; }],
  ["mzkyzak-winner", (buf) => { winnerThumbBuffer = buf; }],
];
...
function getRpgContextInfo(title, body) {
  ...
  if (title || body) {
    base.externalAdReply = {
      title: title || config.bot?.name || "Mzkyzak RPG",
      body: body || "",
...
function createFakeQuoted(botName = "Mzkyzak-AI", verified = true) {
  return {
    key: {
      fromMe: false,
