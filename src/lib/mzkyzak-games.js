import {
  ...
  getRandomReward,
  getProgressiveHint,
} from "./mzkyzak-game-data.js";
import { getDatabase } from "./mzkyzak-database.js";
import { addExpWithLevelCheck } from "./mzkyzak-level.js";
import {
  getGameContextInfo,
  sendGamePreview,
  checkFastAnswer,
} from "./mzkyzak-context.js";
import config from "../../config.js";
import fs from "fs";
import sharp from "sharp";
let fetchBuffer;
try {
  fetchBuffer = (await import("./mzkyzak-utils.js")).fetchBuffer;
} catch { }
        ...
        if (session && session.gameType === gameType) {
          ...
          await sock.sendPreview(
            m.chat,
            {
              ...
              jpegThumbnail: await sharp(fs.readFileSync(config.assets["mzkyzak2"])).resize(300, 300).toBuffer(),
              previewType: 0,
            },
          );
          return;
...
const games = new MzkyzakGames();

export { MzkyzakGames, games };
