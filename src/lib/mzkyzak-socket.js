...
import { downloadMediaMessage, getContentType } from "mzkyzak";
import { addExifToWebp, DEFAULT_METADATA } from "./mzkyzak-exif.js";
...
import {
  getProfilePicture,
  getProfileBuffer,
} from "./mzkyzak-profile-picture.js";
...
async function extendSocket(sock) {
  ...
  sock.sendStickerPack = async (jid, stickers, m, options = {}) => {
    ...
    const publisher = options.publisher ?? options.author ?? "Mzkyzak-AI";
  ...
  sock.sendButton = async function (
    ...
    if (!options.footer) msg.footer = config.bot?.name || "Mzkyzak-AI";
    if (source) {
      let data = source;
