...
import { addJadibotOwner } from "./mzkyzak-jadibot-database.js";
import { extendSocket } from "./mzkyzak-socket.js";
  ...
  if (usePairing && !state.creds?.registered) {
    try {
      ...
      if (m && m.chat) {
        let thumbnail = null;
        try {
          if (!!getAssetBuffer("mzkyzak2")) {
            thumbnail = getAssetBuffer("mzkyzak2");
          }
        } catch { }
