...
import { getDatabase } from "./mzkyzak-database.js";
...
function getBotMode(db = getDatabase()) {
  const dbMode = db.setting("botMode");
  return dbMode || config.mode || "public";
}

function isPublicGroupOverride(m, db = getDatabase()) {
  const publicGroups = db.setting("publicGroups") || [];
  return m.isGroup && publicGroups.includes(m.chat);
}

function shouldSilenceInGlobalSelfMode(m, db = getDatabase()) {
  if (!m.isGroup || m.fromMe) return false;

  // Owner tetap bisa akses
  if (m.isOwner) return false;

  // Cek apakah grup ini di-private secara individual (selfGroups)
  const selfGroups = db.setting("selfGroups") || [];
  if (selfGroups.includes(m.chat)) {
    // Cek apakah grup ini juga di publicGroups (exception)
    return !isPublicGroupOverride(m, db);
  }

  // Cek gcPrivateAll — bot diam di SEMUA grup
  const gcPrivateAll = db.setting("gcPrivateAll") || false;
  if (gcPrivateAll) {
    return !isPublicGroupOverride(m, db);
  }

  // Cek mode self/private global (legacy)
  const mode = getBotMode(db);
...
function checkMode(m, getActiveJadibots) {
  ...
  // Handle gc mode (Group Only)
  if (mode === "gc") {
    if (m.fromMe || m.isOwner) return { allowed: true };
    if (!m.isGroup) return { allowed: false };
  }

  // Handle private mode (Private Chat Only)
  if (mode === "private") {
    if (m.fromMe || m.isOwner) return { allowed: true };
    if (m.isGroup) return { allowed: false };
  }
  ...
  if (mode === "self") {
    ...
    // Mode self global: bot diam di semua grup (kecuali publicGroups)
    if (m.isGroup) {
      return { allowed: false, isGlobalSelfGroup: true };
    }
