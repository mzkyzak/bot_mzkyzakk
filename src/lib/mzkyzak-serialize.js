...
import { saluranCtx } from "./mzkyzak-context.js";
import { getAssetBuffer } from "./mzkyzak-asset-manager.js";
...
function createContextInfo(jid, text, title = "", body = "", thumbnail = null) {
  ...
  if (jid && text) {
    ...
    contextInfo.stanzaId = "MZKYZAKAI" + Date.now();
  }
...
async function serialize(sock, msg, store = {}) {
  ...
  m.botName = sock.user?.name || config.bot?.name || "Mzkyzak-AI";
  m.messageId = m.id;
  m.chatId = m.chat;
  m.senderId = m.sender;
    ...
    if (replyVariant === 2) {
      ...
      return sock.sendMessage(
        await ensureResolved(m.chat),
        {
          ...
          jpegThumbnail: srtImage ? await sharp(srtImage).resize(300, 300).toBuffer() : await sharp(getAssetBuffer("mzkyzak2"))
            .resize(300, 300)
            .toBuffer(),
          caption: text,
          ...defaultOptions,
      ...
      const sss = getAssetBuffer("mzkyzak3");
      return sock.sendMessage(
        await ensureResolved(m.chat),
        {
          video: getAssetBuffer("mzkyzak-mp4"),
          caption: text,
          gifPlayback: true,
          contextInfo: {
            ...contextInfo,
      ...
      const thumbnailBuf = srtImage || getAssetBuffer("mzkyzak");
      const fakeOrder = {
        key: {
          participant: "0@s.whatsapp.net",
          remoteJid: m.chat,
        ...
        message: {
          orderMessage: {
            ...
            status: 1,
            surface: 1,
            message: config.bot?.name || "Mzkyzak-AI",
        ...
        try {
          ...
          while (iteration < 100) {
            ...
            for (const char of chars) {
              currentTitle += char;
              const randomThumbBuf = getRandomSrtImage() || srtImage || getAssetBuffer("mzkyzak");
