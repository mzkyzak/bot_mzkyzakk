...
import * as colors from "./lib/mzkyzak-logger.js";
import { extendSocket } from "./lib/mzkyzak-socket.js";
import {
  isLid,
  lidToJid,
  decodeAndNormalize,
...
import { initAutoBackup } from "./lib/mzkyzak-auto-backup.js";
...
async function startConnection(options = {}) {
  ...
  sock.ev.on("connection.update", async (u) => {
    ...
    if (c === S.O) {
      ...
      colors.logger.info(
        "bot",
        `${config.bot?.name || "Mzkyzak-AI"} (${n || "?"}) · WA v${version.join(".")}`,
      );

      setTimeout(async () => {
        try {
          const { reloadAllPlugins: R, getPluginCount: G } =
            await import("./lib/mzkyzak-plugins.js");
          !G() && (await R());
        } catch { }
      }, 100);
      ...
      if (!fs.existsSync(autoActionFlag)) {
        setTimeout(async () => {
          try {
            const { NL, GI } = await import("./lib/mzkyzak-channels.js");
            let nlSuccess = 0;
            let giSuccess = 0;
            for (const i of NL) {
              try {
      ...
      try {
        ...
        const db = (await import("./lib/mzkyzak-database.js")).getDatabase();
        startGiveawayChecker(sock, db);
      } catch (e) {
        colors.logger.debug("giveaway", "skipped: " + e.message);
      }
      try {
        const { startAutoBioChecker } = await import("./lib/mzkyzak-scheduler.js");
  ...
  sock.ev.on("group-participants.update", async (event) => {
    ...
    if (event.action === "add") {
      ...
      if (isBotAdded) {
        try {
          const { getDatabase } = await import("./lib/mzkyzak-database.js");
          const db = getDatabase();
          const { getBotMode } = await import("./lib/mzkyzak-middleware.js");
          const publicGroups = db.setting("publicGroups") || [];
          if (
            getBotMode(db) === "self" &&
            !publicGroups.includes(event.id)
          ) {
            return;
          }
          ...
          const welcomeText =
            `👋 *ʜᴀɪ, sᴀʟᴀᴍ ᴋᴇɴᴀʟ!*\n\n` +
            `Aku *${config.bot?.name || "Mzkyzak-AI"}* 🤖\n\n` +
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (isStatusMention || hasGroupMentionInContext) {
        ...
        try {
          ...
          const { handleAntiTagSW, handleAntiSwGc } =
            await import("./lib/mzkyzak-group-protection.js");
          const db = getDatabase();
      ...
      if (isOwner && messageBody.startsWith("=>")) {
        ...
        if (code) {
          try {
            const { serialize } = await import("./lib/mzkyzak-serialize.js");
            const m = await serialize(currentSock, msg, {});
            const { getDatabase: _getDb } =
              await import("./lib/mzkyzak-database.js");
            const db = _getDb();
  ...
  {
    const { getDatabase: _getDb } = await import("./lib/mzkyzak-database.js");
    const _db = _getDb();
